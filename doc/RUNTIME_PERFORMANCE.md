...
## Player pathfinding load (2026-09-23)

실제 로컬 건물·가구·던전 데이터로 초당 1,000/5,000건의 공유 Rust 경로 탐색을 측정했다.
[서버 주도 플레이어 이동 설계·측정 문서](SERVER_PLAYER_MOVEMENT.md)에 작업자 수별 지연,
요청 누락, CPU 비용과 재실행 명령을 기록했다. 운영 `c8i.2xlarge`에서도 작업자 4개로
1,000/s를 5초씩 측정해 혼합·던전 구성 모두 누락 0, p99 약 1.4ms를 확인했다.
운영 서버 전체의 동접 부하 시험은 아니다.
같은 날 공개 프로덕션 주소의 시작 지연도 분해했다. 현재 머신의 WebSocket RTT는 평균
4.8ms, 외부 TCP 측정 지점의 평균은 서울 3–4ms, LA 131–184ms, 뉴욕 201–209ms였다.
[시작 지연 절](SERVER_PLAYER_MOVEMENT.md#클릭부터-이동-시작까지의-지연)에 실측과
서버 추가 대기 20/50ms를 가정한 계산을 구분했다. 지역별 지점은 데이터센터 네트워크다.
1단계는 입력 즉시 서버 A*를 시작하고 클라는 승인 경로를 받은 뒤 출발하는 구조다.
실제 조작 반응이 충분하면 유지하고, 부족할 때만 예측 출발·불일치 보정을 2단계로
검토한다. 게임 서버 통합 부하와 클릭부터 이동 표시까지의 지연은 아직 검증하지 않았다.
아래 2026-07-21 절의 "서버는 A*를 실행하지 않는다"는 설명은 당시 상태이며,
현재는 몬스터 AI와 플레이어 경로 재연결에서도 서버 탐색을 사용한다.
