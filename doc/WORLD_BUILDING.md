...
- `v0.40` (2026-05-28): 엘미르 해 섬 `세린 섬(Serinisle)` 명명·지도 표기 — 작은 달 세린에서 따온 이름으로, 큰 달 엘도르 섬과 나란한 '두 달의 섬' 쌍을 이룸(봉우리 205m, 20.5k셀). 지도 라벨(`data/map_labels.csv`)·섬 표 추가
- `v0.39` (2026-05-28): 엘미르 해 섬 `여울 섬(Rillisle)` 명명·지도 표기 — 앨더마크 앞바다에 뜬, 물길이 가장 많은 서쪽 섬(29.4k셀). 지도 라벨(`data/map_labels.csv`)·섬 표 추가
- `v0.38` (2026-05-28): 여명 해 남단 섬 `남끝 섬(Vergeisle)` 명명·지도 표기 — Springisle 남쪽 이웃(35.9k셀)으로 봉우리·물줄기를 갖춘 세계 남변의 마지막 섬. 지도 라벨(`data/map_labels.csv`)·섬 표 추가
- `v0.37` (2026-05-28): 여명 해 남부 섬 `샘 섬(Springisle)` 명명·지도 표기 — 중앙 봉우리에서 시냇물이 흐르는 담수 보급지. 워드젠 수정(`small_island_hill_max_cells` 40k→80k)으로 이 46k셀 중형 섬에 산·강이 생긴 것을 반영. 지도 라벨(`data/map_labels.csv`)·섬 표 추가
...
- `v0.32` (2026-05-28): 외세 설정 추가 — 북방 산악 대륙 아이름에 교역왕국 `하브가르드 (Havgard)` 신설(교역 군주정/선주 회의, 수도 스텐하벤, 아이름이 감싸 안은 잔잔한 내해 `은빛 만(Silverbight)`을 무대로 한 해상 교역 문화, 유물 암거래로 가라스의 눈·유적 관리국과 충돌, 현 교역왕 시그리드). 은빛 만 패권을 다투는 두 항구 — 수도 `스텐하벤`과 신흥 자유 항구 `프리하벤(Frihavn)` — 의 내부 경쟁 구도 설정(에드라 vs 스텐하벤 구도는 에드라가 은빛 만에 접하지 않아 폐기). 가라스와의 공식 교역 창구로 에드라 건너편의 관문 항구 `브로비크(Brovik)` 추가. 기존 `흑해만(Darkbight)`을 은빛 만으로 개명·재정의(위험한 만 → 아이름이 둘러싼 안전한 교역 내해), 스텐하벤·프리하벤·브로비크를 지도 라벨(`data/map_labels.csv`)에 추가, 대륙·도시·바다 표 갱신
- `v0.31` (2026-05-28): 지명 라벨을 공유 데이터(`data/map_labels.csv`, 월드 좌표)로 단일화 — doc 지도(map.png)와 게임 내 세계 지도(M키)가 같은 데이터를 사용. 게임 내에서는 이미지에 굽지 않고 HTML/CSS 오버레이로 표기하며 줌 단계별로 필터링(멀리=대륙/바다, 가까이=도시/마을)
