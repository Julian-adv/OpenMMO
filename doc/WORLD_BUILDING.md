...
- 월 이름(1월~12월):
  ...
  한국어·일본어·중국어 이름은 기존 의미를 살린 의역이다. 한국어는 `달`, 일본어는 `の月` 또는 `月`, 중국어는 `月`로 끝맺는다.
  날짜 표기 예: `가라스 기원 217년 울림달 15일`. 천체 시계에는 `217년 울림달 15일`, `217年 響きの月 15日`, `217年共鸣月15日`처럼 표시한다.

  | 월 | 영문 이름 | 한국어 | 일본어 | 중국어(간체) | 의미 |
  |---|---|---|---|---|---|
  | 1월 | `Dawnmere` | 새벽달 | 暁の月 | 黎明月 | 새벽의 고요 |
  | 2월 | `Reson` | 울림달 | 響きの月 | 共鸣月 | 공명의 시기 |
  | 3월 | `Verdant` | 신록달 | 新緑の月 | 新绿月 | 생장이 무성한 시기 |
  | 4월 | `Highsun` | 한낮달 | 真昼の月 | 高阳月 | 태양이 가장 높은 시기 |
  | 5월 | `Emberfall` | 잔불달 | 残り火の月 | 余烬月 | 불씨가 식어가는 시기 |
  | 6월 | `Redrain` | 붉은비달 | 紅雨の月 | 红雨月 | 붉은 비의 시기 |
  | 7월 | `Harvestwind` | 거둠달 | 実りの月 | 丰收月 | 수확 바람의 시기 |
  | 8월 | `Gloam` | 땅거미달 | 黄昏の月 | 暮光月 | 황혼이 길어지는 시기 |
  | 9월 | `Riftwane` | 틈쉼달 | 裂け目の眠る月 | 裂隙沉寂月 | 균열의 기세가 약해지는 시기 |
  | 10월 | `Mistveil` | 안개달 | 霧隠れの月 | 雾幕月 | 안개 장막의 시기 |
  | 11월 | `Frostrest` | 서리쉼달 | 霜休みの月 | 霜憩月 | 서리와 휴식의 시기 |
  | 12월 | `Afterglow` | 잔빛달 | 残光の月 | 余辉月 | 잔광이 남는 시기 |
...
게임 내 지도 지명은 영어 원명의 발음을 옮겨 표기한다. 기존 음역(발드란, 앨더마크 등)은 유지하며, 뜻을 옮긴 설정상의 별칭은 지도 라벨로 사용하지 않는다. 예: `Silverbight → 실버바이트`, `Ashisle → 애시아일`, `Mistward Sea → 미스트워드 해`. `Sea`처럼 고유명과 분리된 지형 일반명사는 각 언어로 번역한다. 던전명도 `Old Crypt → 올드 크립트`처럼 음역한다. 언어별 표기는 `client/src/lib/i18n/locales/*.places.json`에서 관리한다.
...
- `v0.42` (2026-09-22): 12개월의 한국어 월 이름 확정, 일본어·중국어 의역 및 천체 시계의 언어별 날짜 표기 추가.
