...
공격 간격의 **기준값**은 원거리도 근접과 같은 `player_attack_interval`(1380)이다.
다만 궁술 숙련이 이 값을 최대 30%까지 줄인다(아래 "궁술"). 숙련 0인 사수는
근접과 정확히 같은 박자로 쏜다. 클립이 간격보다 짧아 끝 자세로 잠깐 머무는 것은
slash4(1.3초)도 마찬가지다.
...
- 활의 실제 월드 좌표에서 출발한다(`PlayerModel.getBowWorld`). 발사 요청은
  ...
  프레임에 꺼내 쏜다.
- 사수당 한 발이면 충분하다 — 가장 짧은 공격 간격(궁술 30레벨의 1062 ms)도
  최장 비행 333 ms보다 훨씬 길다.
...
숙련 스킬(`shared/src/skills.rs`, 레벨 0~30)의 두 번째 소비자다. **`range`를
선언한 무기에만** 붙는다 — `rangedAbility`가 아니라 `range`가 기준이라, 장차
쇠뇌나 투척 무기가 들어와도 그대로 적용된다. 칼은 아무리 숙련이 높아도 빨라지지
않는다.
...
```
archery_attack_mult(level)  = 1.0 + 0.01 · level     (30레벨 = 1.30)
archery_damage_bonus(level) = min(level / 6, 5)      (30레벨 = +5)
```

**속도 상한 30%는 애니메이션이 정한 값이다.** 30레벨의 1062 ms는 `bow_shoot`
클립 길이(`player_ranged_clip` 1033 ms)보다 여전히 길어서, 클립·소리·화살 비행
타이밍을 하나도 건드리지 않는다. 이 선을 넘기면 발사 동작이 중간에 잘리고
`player_ranged_impact`(780 ms, 서버의 전리품 지연도 이 값을 읽는다)까지 함께
움직여야 한다. 회귀 방지로
`the_capped_archery_cadence_still_outlasts_the_bow_clip`이 이 부등식을 지킨다.

**피해 상한 +5는 만렙 인챈트 무기와 같다**([ENCHANT.md](ENCHANT.md)). 능력치
보정·인챈트와 같은 자리에 더해지는 평범한 정수 보정이라, 이 시스템에 없는
백분율 계산을 새로 들이지 않는다. **명중률에는 관여하지 않는다** — 폭발 주사위
때문에 명중 보너스는 높은 guard 구간에서 초선형으로 작용하므로, 숙련의 강도
예산은 속도와 피해로만 쓴다.

30레벨 사수의 DPS는 0레벨 사수의 약 2.17배다. 근접에는 대응하는 스킬이 아직
없으므로, 후반부에서 원거리가 근접보다 뚜렷이 세다. 근접 스킬이 생기기 전까지의
의도된 상태다.

#### 경험치

명중한 원거리 공격 한 번에 **고정 10 XP**. 빗나가면 0이고, 처치에 보너스는
없으며, 상대 레벨도 보지 않는다 — 초보와 고수가 같은 속도로 는다. 30레벨까지
945,500 XP이므로 순수 원거리 전투 기준 약 66시간이다. 고정값이라 "안 죽이고
계속 쏘는" 표적 연습이 가장 효율적인데, 이는
[REPEAT_FARMING.md](REPEAT_FARMING.md)의 "많이 하는 것 자체는 문제가 아니다"
입장에 따라 제동을 걸지 않는다.

XP는 명중 즉시 `pending_archery_xp`에 쌓아 두고 5초마다 `tick_archery_xp`가
한꺼번에 넘긴다. 명중마다 전역 스킬 쓰기 잠금을 잡고 메시지를 보내면 동시
5,000명에서 초당 3,600건의 잠금 경합이 되고, 전투 로그도 1.4초마다 한 줄씩
덮인다. 대신 **레벨업은 넘기는 순간에야 발견되므로** 최대 5초 늦는다.
로그아웃 경로(`take_player_skills`)는 넘기고 나서 스냅샷을 뜬다.

#### 장비 요구 스킬

`items.csv`의 `minSkill`/`minSkillLevel` 한 쌍이 착용을 막는다. 서버 권위 판정은
`equip_item`에 있고, 잘못된 스킬 토큰은 부팅 단언에서 걸린다. **지금은 모든
행이 비어 있다** — 활은 하나뿐이라 잠글 대상이 없다. 장차 상위 활·쇠뇌가
들어올 때 CSV 두 칸만 채우면 된다.
