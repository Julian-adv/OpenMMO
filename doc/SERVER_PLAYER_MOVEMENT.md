...
### 전역 이동 락 측정 (2026-09-25)

5,000명 이동 중 입력을 넣는 시험에서 키보드 갱신도 `goal_moves`를 기다리는 것을 확인했다.
캐시가 준비된 키보드 이동은 틱 중앙값 약 66ms, 갱신 처리 p95 약 56ms였다.
첫 높이 타일 읽기에 20ms 지연을 넣으면 다른 플레이어의 입력 처리도 약 22~23ms 늦어졌다.
측정한 360틱 모두 200ms 이내였지만, 전역 락이 입력 지연을 전파한다는 근거는 확인됐다.
...
아래 값은 **각 실행에서 구한 통계값 세 개의 중앙값**이며, 단위는 ms다.

| 조건 | 틱 p50 | 틱 p95 | 갱신 p95 | 방향 변경 p95 | 정지 p95 |
|---|---:|---:|---:|---:|---:|
| 클릭, 캐시 준비 | 13.87 | 18.70 | 3.96 | 18.71 | 18.38 |
| 키보드 보행, 캐시 준비 | 65.93 | 68.27 | 55.80 | 68.11 | 67.68 |
| 키보드 말 탑승, 캐시 준비 | 69.45 | 76.00 | 62.13 | 75.74 | 75.45 |
| 혼합, 캐시 준비 | 48.16 | 51.70 | 30.67 | 51.64 | 51.13 |
| 혼합, 빈 캐시 | 48.98 | 50.52 | 29.54 | 50.42 | 49.97 |
| 혼합, 빈 캐시 + 첫 읽기 20ms 지연 | 71.86 | 73.03 | 52.39 | 72.82 | 72.35 |

| 조건 | movement_gate 대기 p99 | movement_gate 보유 p99 | goal_moves 대기 p99 | goal_moves 보유 p99 |
|---|---:|---:|---:|---:|
| 클릭, 캐시 준비 | 17.53 | 14.57 | 3.04 | 2.79 |
| 키보드 보행, 캐시 준비 | 67.47 | 66.57 | 55.14 | 55.02 |
| 키보드 말 탑승, 캐시 준비 | 74.71 | 70.20 | 62.23 | 58.23 |
| 혼합, 캐시 준비 | 53.56 | 49.08 | 30.04 | 28.66 |
| 혼합, 빈 캐시 | 49.18 | 49.69 | 28.76 | 29.25 |
| 혼합, 빈 캐시 + 첫 읽기 20ms 지연 | 71.93 | 72.74 | 51.64 | 51.35 |

측정 조건:

- 5,000명을 32m 간격의 평지에 배치한다. 혼합은 클릭 보행·클릭 말 탑승·키보드 보행·
  키보드 말 탑승이 각각 25%다. 실제 요청으로 이동을 준비하고 한 번 진행한 뒤 상태를 저장한다.
- 각 라운드에서 같은 플레이어·경로·공간 셀 상태를 복원하고 경과 시간을 200ms로 설정한다.
  입력 만료가 결과에 섞이지 않도록 테스트의 입력 유효 시간을 60초로 연장한다.
  실제 진행량은 기존 코드대로 플레이어별 처리 시점까지의 경과 시간을 포함한다.
- `advance_goal_players`로 5,000명의 이동을 진행한다. 별도 플레이어 30명이 시작 1ms 후
  갱신·새 방향·정지를 각각 10건 요청한다. 다음 라운드는 이전 요청이 모두 끝난 뒤 시작한다.
- 입력 지연은 예정 시각부터 핸들러 완료까지다. 실제 시작의 스케줄링 지연도 별도로 기록하며
  이번 실행에서는 대체로 1.1ms였다. 방향 변경·정지 응답이 채널에 들어갔는지 확인하고,
  동일 요청 갱신은 응답을 만들지 않는지 확인한다. 네트워크 왕복 시간은 포함하지 않는다.
- 캐시 준비 조건은 측정 중 타일 읽기가 0회임을 검사한다. 빈 캐시는 높이 버전을 바꾸지 않고
  샘플러를 교체해서 만든다. 따라서 지형 편집에 따른 클릭 경로 높이 재계산은 포함하지 않는다.
  빈 캐시 조건의 읽기는 실행당 12,800회였으며, 데이터는 메모리의 평지 배열이다.
- 지연 조건은 라운드마다 첫 타일 읽기 한 번만 20ms 대기시킨다. 관측한 실제 대기는 약 21.2ms다.
  나머지 읽기는 지연 없는 메모리 데이터 반환이다. 실제 디스크 성능을 측정한 결과가 아니다.
- 락 계측은 테스트 전용 래퍼로 모든 획득의 대기·보유 시간을 기록한다. 테스트 밖에서는
  기존 Tokio Mutex를 그대로 사용한다. 락 통계에는 틱과 입력 핸들러의 획득이 함께 들어가며,
  중첩 락의 대기·보유 시간은 서로 더해서 해석하지 않는다.

이번 시험은 이동 정산 핵심을 분리한 시험이다. `validate_mounts`, 지속적인 입력 도착,
밀집 지역의 실제 AOI 수신자, WebSocket/TLS, 전투·AI·자연 스폰과 실제 지형 I/O는 제외했다.
라운드별 초기화로 틱 지연이 다음 틱에 누적되는 상황도 측정하지 않는다. p95/p99는 이 짧은
조건부 시험의 분포이며, 서비스 전체의 입력 지연 분포나 5,000명 수용량을 뜻하지 않는다.

우선 개선 후보는 `goal_moves`를 보유한 높이 읽기·예측 계산이다. 갱신은 `movement_gate`를
건너뛰어도 이 락에서 수십 ms 대기한다. 방향 변경과 정지는 이동 후처리가 끝날 때까지
`movement_gate`도 기다리므로, 내부 락만 나누는 변경의 효과는 이 시험으로 다시 확인해야 한다.

### 키보드 예측 계산 분리 (2026-09-25)

키보드의 최대 400ms 예측을 `movement_gate`와 `goal_moves` 밖으로 옮겼다.
위치 정산 중 플레이어·방향 입력·이동 세대·속도를 복사하고, 이동 후처리와 락 해제를 마친 뒤
예측 계산과 메시지 직렬화를 수행한다. 실제 위치 정산의 높이 읽기와 충돌 검사는 기존 락 안에 남는다.

전송 직전에 짧게 락을 다시 잡고 이동 세대·현재 위치/회전/층·탑승·생존/상호작용 상태·
입력 만료·높이 버전·통행 정보 변경을 검증한다. 검증과 채널 enqueue 사이에는 await가 없다.
방향 이동을 정산할 때마다 이동 세대를 올려, 같은 요청 번호의 이전 틱 예측도 폐기한다.
`server_time_ms`는 예측에 쓰는 위치를 복사할 때 기록한다.

새 방향·클릭 목표·정지 요청은 기존 위치만 정산하고 곧 교체할 입력의 예측을 기다리지 않는다.
문 처리도 주변 이동 정산과 문 상태 변경을 같은 락으로 보호한 뒤 락을 풀고 예측한다.
백그라운드 예측 태스크를 추가하지 않았으며, 호출자는 자신의 예측 완료를 계속 기다린다.
...
|---|---:|---:|---:|---:|
| 클릭, 캐시 준비 | 18.70 → 19.04 | 3.96 → 3.05 | 18.71 → 18.71 | 18.38 → 18.33 |
| 키보드 보행, 캐시 준비 | 68.27 → 69.50 | 55.80 → 17.76 | 68.11 → 29.67 | 67.68 → 29.45 |
| 키보드 말 탑승, 캐시 준비 | 76.00 → 72.29 | 62.13 → 19.25 | 75.74 → 31.29 | 75.45 → 31.10 |
| 혼합, 캐시 준비 | 51.70 → 49.29 | 30.67 → 9.42 | 51.64 → 28.19 | 51.13 → 27.97 |
| 혼합, 빈 캐시 | 50.52 → 51.59 | 29.54 → 9.69 | 50.42 → 28.93 | 49.97 → 28.73 |
| 혼합, 빈 캐시 + 첫 읽기 20ms | 73.03 → 74.53 | 52.39 → 33.23 | 72.82 → 53.20 | 72.35 → 52.85 |

키보드 보행의 갱신 지연은 약 68%, 정지 지연은 약 56% 감소했다. 전체 틱 시간은 비슷하다.
예측 계산량을 줄인 변경이 아니라 다른 입력이 기다리는 구간을 줄인 변경이다.
지연을 넣은 빈 캐시 조건은 실제 위치 정산 중의 첫 읽기를 지연시키므로 여전히 그 비용이 남는다.

예측 전송마다 짧은 락 획득이 추가되어 전체 락 획득의 p99는 변경 전과 직접 비교하기 어렵다.
대신 실행별 **가장 긴 보유 시간의 중앙값**을 비교하면 다음과 같다.

| 조건 | movement_gate 보유 최대 | goal_moves 보유 최대 |
|---|---:|---:|
| 키보드 보행, 캐시 준비 | 69.29 → 31.27 | 57.18 → 19.31 |
| 키보드 말 탑승, 캐시 준비 | 76.37 → 32.36 | 64.25 → 20.31 |
| 혼합, 캐시 준비 | 55.40 → 28.81 | 32.06 → 10.37 |
| 혼합, 빈 캐시 + 첫 읽기 20ms | 73.14 → 54.50 | 53.45 → 34.22 |

검증: 서버 release 전체 1,010개 통과, 8개 기본 제외. 위 부하 시험은 별도로 3회 통과했다.
새 회귀 테스트는 예측 높이 읽기를 멈춘 동안 정지·방향/클릭 교체가 진행되는지,
이전 요청과 같은 요청의 이전 틱 예측이 뒤늦게 전송되지 않는지 확인한다.
지형·통행 정보·위치·탑승·생존·접속 상태가 달라진 예측의 폐기도 확인했다.
`cargo fmt --all -- --check`, `cargo check --workspace --all-targets`를 통과했다.
이 시험의 네트워크·AOI·지속 부하 제외 조건은 앞선 측정과 같다.

### 캐릭터·지역별 동기화 분리 (2026-09-25)

전역 `movement_gate`와 전체 `goal_moves` 맵을 보호하던 비동기 Mutex를 제거했다.
접속한 `PlayerId`마다 이동 상태 Mutex를 만들고, 짧은 동기식 레지스트리 락은
해당 Mutex의 참조를 찾을 때만 사용한다. 레지스트리 락을 잡고 await하지 않는다.
접속 종료는 진행 중인 이동을 기다린 뒤 상태와 레지스트리를 제거한다.
종료된 ID로 늦게 실행되는 작업은 레지스트리를 다시 만들지 않는다.

캐릭터 락은 위치 계산·반영과 공간 인덱스·층 변경·전송 순서를 보호한다.
텔레포트, 부활, 가구 상호작용, 탑승 변경도 같은 락을 사용한다.
클릭 이동은 플레이어를 복사해 계산한 뒤 위치·회전·층만 짧게 반영한다.
전체 플레이어 맵의 쓰기 락을 5,000명 계산 내내 잡지 않으며,
동시에 바뀐 체력 등 다른 필드는 덮어쓰지 않는다.

지역 락은 지상 **128m × 128m**, 던전은 **입구 ID별 전 층 하나**인 RwLock이다.
지상 X 셀은 `WORLD_MIN_X`에서 시작하는 256개 순환 셀이며 Z 방향은 128m 간격이다.
일반 이동은 읽기 락이므로 같은 지역의 다른 캐릭터도 동시에 이동할 수 있다.
문 변경과 가구 점유·부활 침대 선택은 쓰기 락을 사용한다.
던전 발자국과 겹치는 지상 셀도 던전 락을 함께 잡는 보수적인 구성이다.

경계에서는 몸 반경 0.31m를 포함한 경로 범위의 모든 지역을 잡는다.
클릭은 남은 경로의 경계 상자, 키보드는 실제 정산 시간과 최대 속도로 계산한
도달 가능 범위를 사용한다. 텔레포트는 출발지·도착지만 포함한다.
여러 지역은 항상 정렬된 순서로 잡고, **지역 락 → 캐릭터 락** 순서를 지킨다.
획득 후 현재 위치·이동 상태로 필요한 지역을 다시 계산해 부족하면 모두 풀고 재시도한다.
캐릭터 락을 잡은 채 지역 락을 기다리거나 읽기 락을 쓰기 락으로 승격하지 않는다.

문은 주변 이동을 먼저 정산하고 해당 지역의 쓰기 락을 획득한 뒤 현재 몸 위치를
다시 검사한다. 그 사이 완료된 이동도 점유 검사에 반영되며, 닫힌 뒤 대기에서 풀린
이동은 닫힌 문으로 충돌 검사한다. 지역 락은 위치 반영 후 해제하므로
AOI·지형 전송 준비의 I/O까지 같은 지역의 이동을 막지 않는다.
예측 계산은 계속 락 밖에서 실행하고, 전송 직전에는 예측 범위의 읽기 락과
캐릭터 락을 잡아 세대·위치·통행 정보 등을 검증한다.

이 지역 분리는 이동·문·캐릭터 재배치의 동기화 범위다. 일반 맵 편집의 저장 락과
통행 정보 스냅샷·높이 버전 갱신은 기존 구조를 사용한다.
틱의 플레이어 순회를 병렬 작업 큐로 바꾸지는 않았다. 한 틱 자체의 지연과
다른 요청이 전역 이동 락 때문에 대기하는 문제를 구분해 측정한다.

구현: [synchronization.rs](../server/src/game_state/goal_movement/synchronization.rs).
회귀 테스트는 같은 지역의 다른 캐릭터 처리, 같은 캐릭터의 이동·텔레포트·접속 종료,
지역 경계 대기 중 텔레포트, 월드 X 연결부, 던전 입구·층 간 배타 처리를 확인한다.
...
| 조건 | 틱 p95 | 갱신 p95 | 방향 변경 p95 | 정지 p95 |
|---|---:|---:|---:|---:|
| 클릭, 캐시 준비 | 19.04 → 23.24 | 3.05 → 1.32 | 18.71 → 1.56 | 18.33 → 1.48 |
| 키보드 보행, 캐시 준비 | 69.50 → 77.20 | 17.76 → 1.33 | 29.67 → 1.60 | 29.45 → 1.50 |
| 키보드 말 탑승, 캐시 준비 | 72.29 → 80.51 | 19.25 → 1.36 | 31.29 → 1.58 | 31.10 → 1.50 |
| 혼합, 캐시 준비 | 49.29 → 57.77 | 9.42 → 1.34 | 28.19 → 1.64 | 27.97 → 1.55 |
| 혼합, 빈 캐시 | 51.59 → 59.28 | 9.69 → 1.27 | 28.93 → 1.62 | 28.73 → 1.53 |
| 혼합, 빈 캐시 + 첫 읽기 20ms | 74.53 → 84.08 | 33.23 → 1.66 | 53.20 → 1.96 | 52.85 → 1.90 |

키보드 보행 중 다른 캐릭터의 정지 p95는 약 95%, 지연된 빈 캐시 조건에서는 약 96%
줄었다. 높이 읽기가 느린 캐릭터는 자기 락을 계속 보유하지만 같은 지역의 다른 캐릭터는
지역 읽기 락을 함께 획득할 수 있다. 지연 조건의 캐릭터 락 최대 보유 시간 중앙값은
22.13ms였으며, 그 캐릭터 본인의 요청이나 같은 지역의 문 변경은 이를 기다릴 수 있다.

틱 p95는 4.20–9.55ms 증가했다. 캐릭터별 락·지역 조회·재검증 비용이 추가되었으며,
이 수치는 모든 락 획득·해제를 기록하는 테스트 계측 비용도 포함한다.
360개 틱 중 200ms를 넘은 틱은 없었고 최대는 85.67ms였다.
부하 시험에는 문 동시 변경·실제 네트워크·AOI 수신자·전투가 없으며,
지역 쓰기 락과 경계의 정확성은 별도 동시성 테스트로 검증했다.

검증: `cargo fmt --all -- --check`, `cargo check --workspace --all-targets`,
서버 release 전체 **1,018개 통과**, 8개 기본 제외. 5,000명 부하 시험은 별도로 3회 통과했다.
