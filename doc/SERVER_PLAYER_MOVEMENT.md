# 서버 주도 플레이어 이동: 설계 방향과 측정 결과

2026-09-23 논의 정리. **1단계는 서버가 공식 경로·위치를 결정하고, 클라이언트는
서버 승인 후 출발하는 구조**로 구현한다. 조작 반응이 충분하면 이 구조를 유지한다.
**예측 출발은 필요할 때만 구현하는 선택적 2단계**다. 아래는 설계와 검증 계획이며
새 이동 프로토콜은 야외·던전의 일반 도보 클릭에 연결했다. 나머지 이동 방식의 전환은
진행 중이다. 경로 탐색 벤치마크의 기준 소스는 `17ba6488`이다.

## 구현 진행

설계·벤치마크를 `36eed5b4`, 정리한 서버 탐색 기반을 `e1da4780`에 저장했다.
그 다음 변경에서 일반 도보 클릭의 요청·탐색·승인·표시를 연결했다.

- 완료: A* 종료 사유를 `reached`, `unreachable`, `node_limit`으로 구분하고 부분 경로를 유지한다.
- 완료: 플레이어 경로 탐색 작업 풀에 실행 4개·대기 128개·대기 기한 100ms를 적용한다.
  실행 중인 작업은 호출자가 취소돼도 실제 계산이 끝날 때까지 실행 슬롯을 점유한다.
  이 수치는 초기 구현값이며 기존 벤치마크의 대기열 설정과는 별개다.
- 완료: 탐색은 공유된 읽기 전용 통행 스냅샷을 사용한다. 문·지형 변경 시 필요한 경우에만
  복사하고 스냅샷 식별이 달라지게 해, 탐색이 문 상태 갱신의 쓰기 잠금을 붙잡지 않게 한다.
  계산 후에는 약한 참조로 변경 여부만 확인해 이전 통행 데이터의 보관을 끝낸다.
- 완료: 기존 서버의 교체 경로 탐색에 작업 풀을 연결하고 이동 큐 잠금을 놓은 뒤 탐색한다.
  계산 중에는 해당 플레이어의 이전 계획을 비우며, 새 이동·정지·위치 변경으로 오래된
  결과가 무효화된다. 탐색 중 통행 정보가 바뀐 결과도 적용하지 않는다.
- 완료: `PlayerMoveGoal(request_id, x, z, sprinting)`·`PlayerMoveStop(request_id)`과
  `PlayerMovePath`·`PlayerMoveProgress`를 추가했다. 프로토콜 버전은 97이다.
  승인 경로에는 서버가 계산한 XYZ·층·속도·탐색 종료 사유·서버 시각을 담는다.
  틱 진행 응답에는 공식 위치·다음 지점 인덱스·속도·상태·서버 시각을 담는다.
- 완료: 첫 클릭은 바로 처리하고 후속 탐색은 플레이어별 100ms 간격으로 제한한다.
  실행 중 탐색은 하나, 대기 목표는 최신 하나만 유지한다. 클라 전송도 같은 간격으로
  첫 입력 즉시·후속 최신 입력 방식으로 제한한다. 정지는 제한을 기다리지 않는다.
- 완료: 승인 뒤의 실제 경과 시간만 이동에 사용한다. 새 목표·정지 수신 시에도 이전
  계획을 수신 시점까지 진행한 뒤 교체한다. 이동 거리만큼 허기를 소비한다.
- 완료: 경로의 높이를 최대 1m 간격으로 서버에서 계산하고 전체를 승인 응답에 담는다.
  이동 중에는 지나가는 구간의 셀·경계와 반경 0.3m를 최신 통행 정보로 검사한다.
  막히면 안전한 구간까지만 전진하고 계획을 종료한다. 문이 다시 열려도 자동 재출발하지 않는다.
  급경사 오르막도 진행 단계에서 멈춘다.
- 완료: 던전 계단의 높이와 층 전환은 공식 위치·현재 층으로 계산한다. XZ 요청만으로
  실제 던전 계단을 오르내리는 테스트와 월드 X 경계를 넘는 경로 테스트를 포함한다.
- 완료: 새 클릭 흐름은 클라 A*·예측 출발 없이 승인을 기다린다. 승인된 경로 표시에는
  프레임 누적 시간이 아닌 `performance.now()`를 사용한다. 초기 구현은 응답 수신 시점을
  표시 기준으로 삼고, 마지막 진행 응답 이후 500ms까지만 표시를 진행한다. 응답이 끊기면
  표시를 멈추며, 다음 공식 위치를 받으면 갱신한다. RTT 기반 시계 보정은 아직 적용하지 않았다.
- 완료: 부분 경로의 종료와 목표 도착을 구분한다. 새 흐름에서는 클라 위치 샘플·층 보고로
  공식 이동을 수정하거나 기존 divergence 보정을 시작하지 않는다. 새 목표·정지·기존 이동·
  텔레포트·접속 종료는 오래된 탐색 결과와 표시 경로를 무효화한다.

현재 적용 범위는 **야외·던전의 일반 도보 클릭**이다. 집을 클릭할 때 입구에서 멈추는 규칙,
상호작용 접근, 전투 추적, 키보드, 자동 여행, 탑승은 아직 기존 이동을 사용한다.
두 흐름은 전환 시 이전 계획을 취소하며, 예측 출발 2단계는 구현하지 않았다.

남은 작업:

- 집 입구 정지 규칙과 나머지 이동 방식을 이 경로로 이전하고 기존 이동 큐·보정 코드 제거.
- 문 닫힘 직전의 이동 시각 정산과 문에 몸이 겹친 경우의 닫기 정책. 현재는 전진 시점의
  최신 통행 상태로 차단하며, 문 이벤트 시각을 기준으로 소급 판정하지 않는다.
- 잘못된 정적 목표의 클라 사전 필터 및 빈도를 제한한 서버 진단. 현재는 유한 좌표·거리·
  플레이어 상태를 검사하고, 탐색 실패·부분 경로·과부하를 응답으로 구분한다.
- 이동 중 지형 편집에 따른 높이 재계산, 실제 브라우저 조작감과 한국·미국 지연 환경 검증.
- 5,000명 규모에서 높이 계산·이동 진행·진행 패킷까지 포함한 부하 측정. 앞의 A* 벤치마크는
  이 전체 처리량을 측정한 결과가 아니다.
...
현재는 클라가 경로와 이동을 진행하면서 서버도 별도의 이동 큐를 시뮬레이션한다.
...
현재 클라의 `dungeonManager.updateFromPlayerPosition(x, z)`도 현재 깊이와 계단 축의
진행 위치, 전환 임계값·여유 구간으로 층을 바꾼다. 공유 Rust의 `shaft_run_pos`와
...
기존 이동 감사 history는 문제 재현과 새 방식 비교에 사용한 뒤 출력량을 줄인다.
...
1단계 구현·검증을 먼저 마치고, 결과에 따라 선택적 2단계를 판단한다.
...
이는 **실측 RTT에 가정을 더한 예상치**다. 이동 프로토콜은 아직 구현하지 않았으므로
