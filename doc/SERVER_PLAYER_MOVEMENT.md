...
현재 공유 A*는 목표에 가까운 발견 노드까지의 부분 경로를 반환하는 기능이 있다.
하지만 `PathResult.found == false`에는 경로 없음과 노드 한도 초과가 함께 들어간다.
통합 시 종료 사유를 구분하고 부분 경로의 충돌·끝점을 검증해야 한다. 기존 반환값만으로
도달 불가를 확정하거나 클라 이상으로 분류하지 않는다.
