...
새 이동 프로토콜은 아직 구현하지 않았다. 경로 탐색 벤치마크의 기준 소스는 `17ba6488`이다.

## 구현 진행

설계·벤치마크를 `36eed5b4`에 먼저 저장한 뒤, 서버 탐색 기반부터 구현하고 있다.
...
- 남음: XZ 목표·정지 입력과 승인 경로 응답, 플레이어별 최신 목표 병합·탐색 제한,
  서버의 경로 진행 및 동적 차단 처리, 클라의 승인 후 출발을 연결한다. 키보드·전투·탑승의
  전환과 기존 클라 이동·보정 흐름 제거도 이 통합 뒤에 검증한다.

현재 브라우저는 기존 이동 프로토콜을 사용한다. 아래의 서버 주도 이동 전체가 완성됐다는
뜻은 아니며, 이미 연결한 서버 탐색 기반과 앞으로 연결할 이동 흐름을 구분한다.
