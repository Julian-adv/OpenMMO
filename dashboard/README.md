...
그 아래의 국가별 접속 계정 표는 유니크 계정과 같은 자정 집계에서 선택 기간에 접속한 계정을 접속 IP의 국가별로 나눠 계정 수·비율·접속 횟수를 표시하고 한국 계정 비율을 요약합니다. 현재 접속 열만 조회 시점의 값입니다. 접속 경로 기준이라 거주 국가와 다를 수 있고, 여러 국가에서 접속한 계정은 각 국가에 포함됩니다. IP 대역은 DB-IP Lite(CC BY 4.0)를 사용하며 표 하단에 출처를 표시합니다. 자세한 내용은 [운영 지표](../doc/METRICS.md#국가별-접속-계정)를 참고하세요.
